// 產生插件圖示 PNG(16/32/48/128)與手機版圖示(pwa/icons/)— 用測試瀏覽器把 SVG 截圖成 PNG
import { chromium } from 'playwright';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';

const out = join(dirname(fileURLToPath(import.meta.url)), '..', 'extension', 'icons');
// full = 滿版方形(iPhone 主畫面、Android 自適應圖示會自己裁圓角,透明角會變黑)
const svg = (size, full = false) => `
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 128 128">
  <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
    <stop offset="0" stop-color="#8b5cf6"/><stop offset="1" stop-color="#6d28d9"/>
  </linearGradient></defs>
  ${full ? '<rect width="128" height="128" fill="url(#g)"/>' : '<rect x="4" y="4" width="120" height="120" rx="28" fill="url(#g)"/>'}
...
const pwaOut = join(dirname(fileURLToPath(import.meta.url)), '..', 'pwa', 'icons');
for (const [name, size, full] of [['icon-192.png', 192, false], ['icon-512.png', 512, false], ['maskable-512.png', 512, true], ['apple-touch-icon.png', 180, true]]) {
  await page.setViewportSize({ width: size, height: size });
  await page.setContent(`<html><body style="margin:0;background:transparent">${svg(size, full)}</body></html>`);
  await page.locator('svg').screenshot({ path: join(pwaOut, name), omitBackground: !full });
}
await browser.close();
